// CSKH Time Tracker Backend v1.3.1 – Final
// Author: Mauka Team
...
function signToken(u) {
  return jwt.sign({ uid: u.id, email: u.email, role: u.role }, JWT_SECRET, {
    expiresIn: "30d",
  });
}
...
app.post("/attendance/log", auth, async (req, res) => {
  ...
  if (!startedAt || !endedAt)
    return res.status(400).send("Thiếu timestamp");
  await db.run(
...
app.get("/attendance/today", auth, async (req, res) => {
  ...
  res.json({
    workMs,
    total_hours: Math.floor(workMs / 3600000),
  });
});
...
app.post("/presence/beat", auth, async (req, res) => {
  ...
  const now = Date.now();
  await db.run(
    "INSERT INTO presence(user_id,status,started_at,last_seen) VALUES(?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET status=?,started_at=?,last_seen=?",
...
app.get("/leader/summary", auth, requireRole(["leader", "admin"]), async (req, res) => {
  const from = Number(req.query.from) || Date.now() - 86400000;
  ...
  for (const r of rows)
    perUser.set(r.user_id, (perUser.get(r.user_id) || 0) + (r.work_ms || 0));
  let total_hours = 0;
  for (const v of perUser.values()) total_hours += Math.floor(v / 3600000);
...
// Admin: payroll (VND)
app.get("/admin/payroll", auth, requireRole(["admin"]), async (_req, res) => {
  const from = Number(_req.query.from) || Date.now() - 30 * 86400000;
  const to = Number(_req.query.to) || Date.now();
  ...
  const msMap = new Map();
  for (const a of att)
    msMap.set(a.user_id, (msMap.get(a.user_id) || 0) + (a.work_ms || 0));

  const payroll = users.map((u) => {
    const ms = msMap.get(u.id) || 0;
    const hours = Math.floor(ms / 3600000);
    const pay = hours * (u.salary_per_hour || 0);
    return {
      email: u.email,
      display_name: u.display_name,
      hours,
